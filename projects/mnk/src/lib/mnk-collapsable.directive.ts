...
import { Collapsable, CollapsableOptions } from 'moonkit';

@Directive({ selector: '[libCollapsable]' })
export class CollapsableDirective implements AfterViewInit, OnChanges, OnDestroy {
  ...
  ngAfterViewInit() { this.collapsableInstance = new Collapsable(this.el.nativeElement, this.collapsableOptions); }
